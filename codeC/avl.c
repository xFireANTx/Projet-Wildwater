...
// (Note : Les rotations Dico sont identiques en logique)
...
NoeudAVL_Histo* creer_noeud_histo(char *id, long capacite) {
    ...
    if (noeud->data == NULL) { free(noeud); exit(1); }
...
NoeudAVL_Histo* inserer_station(NoeudAVL_Histo *node, char *id, long capacite, int *h_change) {
    ...
    if (cmp < 0)
        ...
        // MODIFICATION ICI : CUMUL DES VALEURS
        node->data->capacite += capacite; 
        return node;
    }
...
NoeudAVL_Dico* inserer_dico(NoeudAVL_Dico *node, char *id, Troncon *ptr, int *h_change) {
    ...
    if (balance > 1 && strcmp(id, node->fg->id) < 0) return rotation_droite_dico(node);
    if (balance < -1 && strcmp(id, node->fd->id) > 0) return rotation_gauche_dico(node);
